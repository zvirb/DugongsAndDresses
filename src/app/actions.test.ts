...
describe('Server Actions Logging', () => {
  ...
  it('updateHP logs recovery correctly', async () => {
    ...
    expect(prisma.logEntry.create).toHaveBeenCalledWith({
      data: {
        campaignId: 'camp-1',
        content: expect.stringContaining('**Grom** rallies, reclaiming **5** vitality.'),
        type: 'Combat',
      },
    });
  });
  ...
  it('updateInitiative logs correctly', async () => {
    ...
    expect(prisma.logEntry.create).toHaveBeenCalledWith({
      data: {
        campaignId: 'camp-1',
        content: expect.stringContaining('**Grom** seizes the moment with initiative: **15**.'),
        type: 'Combat',
      },
    });
  });
  ...
  it('createCharacter logs correctly', async () => {
    ...
    expect(prisma.logEntry.create).toHaveBeenCalledWith({
      data: {
        campaignId: 'camp-1',
        content: expect.stringContaining('**NewChar** enters the fray.'),
        type: 'Story',
      },
    });
  });
  ...
  describe('advanceTurn', () => {
    ...
    it('advances turn to the next character', async () => {
        ...
        expect(prisma.logEntry.create).toHaveBeenCalledWith({
            data: {
                campaignId,
                content: expect.stringContaining("The spotlight falls upon **Char2**."),
                type: 'Combat',
            }
        });
    });
    ...
    it('loops back to the first character', async () => {
        ...
        expect(prisma.logEntry.create).toHaveBeenCalledWith({
            data: {
                campaignId,
                content: expect.stringContaining("The spotlight falls upon **Char1**."),
                type: 'Combat',
            }
        });
    });
  ...
  it('performAttack logs Critical Hit correctly', async () => {
    ...
    expect(prisma.logEntry.create).toHaveBeenCalledWith({
      data: {
        campaignId: 'camp-1',
        content: expect.stringContaining('**Attacker** executes a **CRITICAL HIT** upon **Target**, dealing **10** damage!'),
        type: 'Combat',
      },
    });
  });
  ...
  it('performAttack logs Critical Miss correctly', async () => {
    ...
    expect(prisma.logEntry.create).toHaveBeenCalledWith({
      data: {
        campaignId: 'camp-1',
        content: expect.stringContaining('**Attacker** suffers a **CRITICAL MISS** against **Target**!'),
        type: 'Combat',
      },
    });
  });
  ...
  it('performSkillCheck logs success correctly', async () => {
    ...
    expect(prisma.logEntry.create).toHaveBeenCalledWith({
      data: {
        campaignId: 'camp-1',
        content: expect.stringContaining('**Hero** attempts a check of **Athletics** against DC **15**: **SUCCESS** (Roll: **18**).'),
        type: 'Roll',
      },
    });
  });
...
describe('New Server Actions', () => {
  ...
  it('loadEncounter updates characters from participants', async () => {
    ...
    expect(prisma.logEntry.create).toHaveBeenCalledWith({
        data: {
            campaignId: 'camp-1',
            content: expect.stringContaining('The stage is set for **Saved Battle**.'),
            type: 'Combat',
        }
    });
  });
