...
describe('Server Actions Logging', () => {
  ...
  it('updateInitiative logs correctly', async () => {
    ...
    expect(prisma.logEntry.create).toHaveBeenCalledWith({
      data: {
        campaignId: 'camp-1',
        content: expect.stringContaining('**Grom** rolls for initiative: **15**.'),
        type: 'Combat',
      },
    });
  });
  ...
  describe('advanceTurn', () => {
    ...
    it('advances turn to the next character', async () => {
        ...
        expect(prisma.logEntry.create).toHaveBeenCalledWith({
            data: {
                campaignId,
                content: expect.stringContaining("The spotlight shifts to **Char2**."),
                type: 'Combat',
            }
        });
    });
    ...
    it('loops back to the first character', async () => {
        ...
        expect(prisma.logEntry.create).toHaveBeenCalledWith({
            data: {
                campaignId,
                content: expect.stringContaining("The spotlight shifts to **Char1**."),
                type: 'Combat',
            }
        });
    });
  ...
  it('performAttack logs Critical Hit correctly', async () => {
    ...
    expect(prisma.logEntry.create).toHaveBeenCalledWith({
      data: {
        campaignId: 'camp-1',
        content: expect.stringContaining('A natural 20! **Attacker** lands a **CRITICAL HIT** on **Target**!'),
        type: 'Combat',
      },
    });
  });
  ...
  it('performAttack logs Critical Miss correctly', async () => {
    ...
    expect(prisma.logEntry.create).toHaveBeenCalledWith({
      data: {
        campaignId: 'camp-1',
        content: expect.stringContaining('Disaster! **Attacker** suffers a **CRITICAL MISS** against **Target**!'),
        type: 'Combat',
      },
    });
  });
  ...
  it('performSkillCheck logs success correctly', async () => {
    ...
    expect(prisma.logEntry.create).toHaveBeenCalledWith({
      data: {
        campaignId: 'camp-1',
        content: expect.stringContaining('**Hero** checks **Athletics** (DC **15**): **SUCCESS** (Roll: **18**).'),
        type: 'Roll',
      },
    });
  });
  ...
  it('castSpell logs and applies condition correctly', async () => {
    ...
    expect(prisma.logEntry.create).toHaveBeenCalledWith({
      data: {
        campaignId: 'camp-1',
        content: expect.stringContaining('**Wizard** casts **Fireball** targeting **Goblin**, imposing **Burning**!'),
        type: 'Combat',
      },
    });
  });
...
describe('New Server Actions', () => {
  ...
  it('loadEncounter updates characters from participants', async () => {
    ...
    expect(prisma.logEntry.create).toHaveBeenCalledWith({
        data: {
            campaignId: 'camp-1',
            content: expect.stringContaining('Encounter **Saved Battle** begins.'),
            type: 'Combat',
        }
    });
  });
